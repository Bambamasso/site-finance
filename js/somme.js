...
let somme =localStorage.getItem("BUDGET_FIXE")?JSON.parse(localStorage.getItem("BUDGET_FIXE")):[];
...
let somation=0;
const s1=somme.length?somme.reduce((a,b)=> Number(a.montant)+Number(b.montant)): 0;
...
// console.log(somme.reduce((a,b)=> Number(a.montant)+Number(b.montant)),s2,s3);
document.querySelector(".prix").innerHTML=somation+(s1+s2+s3)+ " FCFA";